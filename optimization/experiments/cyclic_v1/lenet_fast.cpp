...
#pragma HLS ARRAY_PARTITION variable=pool1_buffer cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=conv1_buffer cyclic factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=conv2_buffer cyclic factor=2 dim=1
    m6_buffer_copy<INPUT_SIZE>(input,input_buffer);
    conv_banked<1,6,28,28,6>(input_buffer,conv1_w,conv1_buffer);
    relu_pool<6,24,24>(conv1_buffer,pool1_buffer);
    conv_banked<6,16,12,12,8>(pool1_buffer,conv2_w,conv2_buffer);
    relu_pool<16,8,8>(conv2_buffer,pool2_buffer);
#endif

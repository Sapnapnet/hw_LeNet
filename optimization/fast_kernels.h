...
// Aggressive Conv1 kernel: use sixteen spatial PEs.  The input is kept as a
// fully partitioned 28x28 register tile so a tile may cross an image row
// without banking collisions.  This is an optional experiment selected by
// LENET_AGGRESSIVE; the ordinary optimized kernel above is unchanged.
#if defined(LENET_AGGRESSIVE) || defined(LENET_AGGRESSIVE12) || defined(LENET_AGGRESSIVE5)
template <int COUT>
void conv1_banked_m16(const data_t in[28][28],
                      const weight_t weights[COUT * 25],
                      data_t out[2][COUT * 24 * 24 / 2]) {
#pragma HLS INLINE off
...
#pragma HLS ARRAY_PARTITION variable=out complete dim=1
    ...
    const int OW = 24;
    const int SP = OH * OW;
    unsigned char oh[M], ow[M];
#pragma HLS ARRAY_PARTITION variable=oh complete dim=1
#pragma HLS ARRAY_PARTITION variable=ow complete dim=1
    systolic_acc_t acc[M][COUT];
#pragma HLS ARRAY_PARTITION variable=weights complete dim=1
#pragma HLS ARRAY_PARTITION variable=acc complete dim=0

    // Tile two complete output rows (2 x 8).  This keeps all sixteen input
    // windows addressable in parallel and avoids the expensive /24 and %24
    // operations that otherwise dominate each tile's setup time.
    for (int row = 0; row < OH; row += 2) {
      for (int col = 0; col < OW; col += 8) {
        for (int m = 0; m < M; ++m) {
#pragma HLS UNROLL
            if (m < 8) {
                oh[m] = row;
...
#pragma HLS PIPELINE II=1
                data_t av[M];
                weight_t wv[COUT];
#pragma HLS ARRAY_PARTITION variable=av complete dim=1
#pragma HLS ARRAY_PARTITION variable=wv complete dim=1
                for (int m = 0; m < M; ++m) {
#pragma HLS UNROLL
                    av[m] = in[oh[m] + kh][ow[m] + kw];
                }
                for (int j = 0; j < COUT; ++j) {
#pragma HLS UNROLL
                    wv[j] = weights[j * 25 + kh * 5 + kw];
                }
                for (int m = 0; m < M; ++m) {
#pragma HLS UNROLL
                    for (int j = 0; j < COUT; ++j) {
#pragma HLS UNROLL
        ...
        for (int j = 0; j < COUT; ++j) {
            for (int m = 0; m < M; ++m) {
#pragma HLS PIPELINE II=1
                const int spatial = row * OW + col + (m < 8 ? m : OW + m - 8);
                const int pos = j * SP + spatial;
                out[pos % 2][pos / 2] = quantize_acc(acc[m][j]);
            }
        }
      }
